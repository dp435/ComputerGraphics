...
public class CameraController {
	 ...
	 * 
	 * Rotate the camera about one or more of its local axes, by modifying
	...
	protected void rotate(Matrix4 parentWorld, Matrix4 transformation, Vector3 rotation) {
		...
		// Ensure rotation in degrees.
		Vector3 rotationDegrees = rotation.mul((float) (Math.PI / 180.0));

		// Create rotation matrices.
		Matrix4 rotationX = Matrix4.createRotationX(rotationDegrees.x);
		Matrix4 rotationY = Matrix4.createRotationY(rotationDegrees.y);
		Matrix4 rotationZ = Matrix4.createRotationZ(rotationDegrees.z);
		Matrix4 rotationMatrix = rotationX.mulBefore(rotationY).mulBefore(rotationZ);

		// Apply rotation transformation.
		if (!this.orbitMode) {
			transformation.mulBefore(rotationMatrix);
		} else {
			Matrix4 toWorldTranslation = Matrix4.createTranslation(
					transformation.clone().invert().mulBefore(parentWorld.clone().invert()).mulPos(new Vector3()));
			Matrix4 orbitTransformation = toWorldTranslation.clone().mulBefore(rotationMatrix)
					.mulBefore(toWorldTranslation.invert());
			transformation.mulBefore(orbitTransformation);
		}
	}
	...
	protected void translate(Matrix4 parentWorld, Matrix4 transformation, Vector3 motion) {
		// TODO#A3#Part 3
		Matrix4 transMat = Matrix4.createTranslation(motion);
		transformation.mulAfter(transMat);
	}
}
