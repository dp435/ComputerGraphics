...
public class Parser {
	...
	private Object parseObject(Class c, String text) {
		...
		else if (Vector3.class.isAssignableFrom(c)) {
			ArrayList tempArray = parseArray(Float.TYPE, text);
			if (tempArray.size() != 3) {
				throw new Error("Vector is not of length 3 ("
						+ tempArray.size() + ")");
			}
			Vector3 result;
			try {
				result = (Vector3) c.newInstance();
			} catch (Exception e) {
				throw new Error("Error instantiating object of class: "
						+ c.getName());
			}
			result.x = (Float) tempArray.get(0);
			result.y = (Float) tempArray.get(1);
			result.z = (Float) tempArray.get(2);
			return result;
		}

		else if (Colorf.class.isAssignableFrom(c)) {
			ArrayList tempArray = parseArray(Float.TYPE, text);
			if (tempArray.size() != 3) {
	...
	private Object parseObject(Class<?> c, Node n) {
		...
		// Check for certain special classes of the current node
				...
				|| c == Float.class || c == Colorf.class || Vector3.class.isAssignableFrom(c)
