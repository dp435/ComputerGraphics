...
public class Scene {
	...
	public void setBackColor(Colorf color) {
		this.backColor.set(color);
	}

	public Colorf getBackColor() {
	...
	public void addTexture(Texture toAdd) {
		textures.add(toAdd);
	}
	...
	private boolean intersect(IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection) {
		...
		// 3) If there was an intersection, check the modified
		// IntersectionRecord to see
		// if the object was hit by the ray sooner than any previous object.
		// Hint: modifying the end field of your local copy of ray might be
		// useful here.
		...
		for (Surface surface : surfaces) {
			if (surface.intersect(tempRecord, rayCopy)) {
				if (anyIntersection)
					return true;

				intersectionFound = true;
				if (tempRecord.t < rayCopy.end) {
					rayCopy.end = tempRecord.t;
